...
Облачный поток Росдомофон раскачивается только пока к нему подключён зритель — запрос снимка «по требованию» заставлял бы поток каждый раз подниматься заново, из-за чего снимки приходили рвано или пустыми. Вместо этого, пока камера добавлена в Home Assistant, интеграция держит фоновый процесс `ffmpeg`, который непрерывно читает поток через собственный прокси и хранит в памяти последний кадр — снимок отдаётся из него мгновенно.

Это требует настроенного в Home Assistant компонента `ffmpeg` (обычно уже включён по умолчанию) и по одному фоновому процессу `ffmpeg` на каждую добавленную камеру, пока она включена в Home Assistant.
